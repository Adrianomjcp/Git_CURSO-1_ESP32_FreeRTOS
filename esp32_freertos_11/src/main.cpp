...
//--------------------------------------------------------------------------------
  // Configure ADC
  //https://github.com/espressif/esp-idf/blob/master/docs/api-reference/peripherals/adc.rst
...
void prvSetupHardware( void ){
  ...
  timerAttachInterrupt(timerOne, &onTimerOne, true); //ontimeone é o endereço de calback
  timerAlarmWrite(timerOne, 500000, true);//a cada 500ms é gerado uma interrupção
...
void vTask1( void *pvParameters ){
  ...
  if( DEBUG )
    vPrintString( "Task1 Inicializada." );
...
void vTask4( void *pvParameters ){
  ...
  if( DEBUG )
    vPrintString( "Task4 Inicializada." );
  for( ;; ){
