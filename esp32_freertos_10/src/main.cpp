...
void vTask4( void *pvParameters ){
  ...
  for( ;; ){
    ...
    /*
     * Leitura ADC - a variação do canal AD não é linear
    */
...
void setup() {
  ...
  xTaskCreatePinnedToCore( vTask3, "Task 3", configMINIMAL_STACK_SIZE+1000, NULL, 2, NULL, CORE_1 );  //display (vai imprimir o conteúdo da leitura do https)
  xTaskCreatePinnedToCore( vTask4, "Task 4", configMINIMAL_STACK_SIZE+1000, NULL, 3, NULL, CORE_1 );  //adc

}

void loop() {   //TASK loop (task do arduino)
