...
QueueHandle_t xQueue; //CRIAÇÃO DA FILA (tem que ser global para que todas as task tenham acesso a fila, isso é um recurso do SO)

void prvSetupHardware( void ){ //função de inicialização de wardware
  Serial.begin( 9600 ); 
...
void vTask1( void *pvParameters ){
  ...
  uxHighWaterMark = uxTaskGetStackHighWaterMark( NULL ); //quanto ainda tenho de memória?

  BaseType_t xStatus;
  vPrintString( "Task1 Init...\n" ); // imprimo essa mensagem
  ...
  for( ;; ){
    ...
    if( (wifiMulti.run() == WL_CONNECTED) ) {
        ...
        int httpCode = http.POST( "{\"value\": 50}" );
        ...
        if( httpCode > 0 ) {
            ...
            if( httpCode == HTTP_CODE_OK ) { //Code 200
                ...
                lValueToSend = 1;  //
                xStatus = xQueueSendToBack( xQueue, &lValueToSend, portMAX_DELAY ); //portMAX_DELAY = fica aguardando infinitamente, a função não sai daqui enquanto não tenhho espaço na fila (vídeo em 9:00)
                if( xStatus != pdPASS ){
...
void setup() {
  ...
  xQueue = xQueueCreate( 5, sizeof( uint32_t ) ); //crio uma fila de 5 posições


//se der erro na fila nesse if mostro que deu merda
  if( xQueue == NULL )
  {
